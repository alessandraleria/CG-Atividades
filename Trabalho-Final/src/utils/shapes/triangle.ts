...
import { cross, dot, unit } from '../vec/vec_3';
...
function barycentric(point1: Point3, point2: Point3, point3: Point3, intersectPoint: Point3): [number, number, number] {
  const v0 = point2.sub(point1);
  const v1 = point3.sub(point1);
  const v2 = intersectPoint.sub(point1);
...
export class Triangle extends Hittable {
  ...
  constructor(vertex1: Point3, vertex2: Point3, vertex3: Point3, material: Material, normals?: Point3[]) {
    ...
    if (normals) {
      this.normals = normals.map((normal) => unit(normal));
    }
  }
  ...
  hit(r: RayT, rayT: Interval): { objectHit: boolean; rec: HitRecord | null } {
    const v1ToV2 = this.getVertex2().sub(this.getVertex1());
    const v1ToV3 = this.getVertex3().sub(this.getVertex1());
    ...
    const edge1 = this.getVertex2().sub(this.getVertex1());
    const vp1 = intersectPoint.sub(this.getVertex1());
    ...
    const edge2 = this.getVertex3().sub(this.getVertex2());
    const vp2 = intersectPoint.sub(this.getVertex2());
    ...
    const edge3 = this.getVertex1().sub(this.getVertex3());
    const vp3 = intersectPoint.sub(this.getVertex3());
